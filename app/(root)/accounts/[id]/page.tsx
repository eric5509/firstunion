    ...
    return (
        <div className="">
            ...
            <div className="flex sticky top-0 p-4 z-30 bg-black gap-5 mt-4">
                {links.map((el, key) => (
                    <p onClick={() => setActive(key)} className={`px-6 ${key === active ? "bg-[darkblue]" : ""} cursor-pointer duration-300 active:scale-95 text-white text-sm font-semibold py-3 rounded-md`}>{el}</p>
                ))}
            </div>
